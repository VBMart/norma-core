...
import { airgradient_open_air_o_1pst, arduino_nicla_sense_env, dmesg, hikmicro, ina226, usbvideo, st3215, motors_mirroring, sysinfo, yahboom_dogzilla_lite, normvla, vesc_trampa, victron_smartsolar_mppt, arduino_nicla_sense_me, dfrobot_rs485 } from '@/api/proto.js';
...
import DfrobotRs485Expanded from '@/components/history/DfrobotRs485Expanded';
...
function tryDecodeProtobuf(rawData: Uint8Array): { decoded: unknown; typeName: string } | null {
  const decoders = [
    ...
    { name: 'dfrobot_rs485.RxEnvelope', decode: () => dfrobot_rs485.RxEnvelope.decode(rawData) },
...
function getAvailableTabs(
  ...
  const isDfrobotRs485 = type === 'dfrobot-rs485' && data instanceof dfrobot_rs485.RxEnvelope;
  ...
  if (isUsbVideo || isUsbVideoTx || isHikmicroThermal || isSt3215 || isSt3215Tx || isMirroring || isVescTrampaTx || isSysinfo || isArduinoNiclaSenseEnv || isArduinoNiclaSenseMe || isIna226 || isDfrobotRs485 || isAirGradient || isVictronSmartSolar || isYahboomDogzillaLite || isNormvla) {
...
export default function ExpandedView({ data, type, rawData, queueId, entryId }: ExpandedViewProps) {
  ...
  const renderVisual = () => {
    ...
    if (type === 'dfrobot-rs485' && data instanceof dfrobot_rs485.RxEnvelope) {
      return <DfrobotRs485Expanded data={data} />;
    }
  ...
  const renderJson = () => {
    ...
    if (type === 'dfrobot-rs485' && data instanceof dfrobot_rs485.RxEnvelope) {
      const dfrobotData = dfrobot_rs485.RxEnvelope.toObject(data, {
        longs: String,
        enums: String,
        bytes: String,
        defaults: true
      ...
      return (
        <div>
          <div className="text-xs text-text-label mb-1">DFRobot RS-485 RxEnvelope JSON:</div>
          <div className="bg-surface-primary p-2 rounded text-xs font-mono text-accent-data overflow-x-auto max-h-64 overflow-y-auto">
            <pre>{JSON.stringify(dfrobotData, null, 2)}</pre>
          </div>
        </div>
      );
    }
